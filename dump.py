...
class DUsersMod(loader.Module):
    ...
    async def ducmd(self, message):
        ...
        f = io.BytesIO()
        f.name = f'Dump by {chat.title}.txt'
        ...
        f.seek(0)
        if tome:
            await message.client.send_file('me', f, caption="Дамп чата " + str(chat.title))
        else:
            await message.client.send_file(message.to_id, f, caption="Дамп чата " + str(chat.title))
        if not silent:
            if tome:
                if num == False:
